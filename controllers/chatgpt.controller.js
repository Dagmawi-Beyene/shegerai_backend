...
async function createCompletionWithChatGPT(req, res) {
	...
	if (!body.messages) {
		res.status(400).json({ error: 'Messages not found' })
		return
	}
	...
	try {
		...
		await usageRef
			.get()
			.then((snapshot) => {
				if (snapshot.exists && snapshot.data().count >= 15) {
...
// export createCompletion function
export default createCompletionWithChatGPT
